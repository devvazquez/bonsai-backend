...
if (-not $env:GROQ_API_KEY -or $env:GROQ_API_KEY -eq "tu-api-key-de-groq") {
    Write-Host ""
    Write-Host "Falta GROQ_API_KEY en el .env: /describe dara error 500." -ForegroundColor Yellow
    Write-Host "El resto (/speak, /memory, /voices) si funcionara." -ForegroundColor Yellow
}
